...
void get_best_string_heur3(std::vector<std::vector<float>> & profile, std::size_t &n, std::size_t &s,
                      std::vector<float>& best_remaining_scores,
                      std::string& best_string, float& best_score,
                      std::string& alphabet, int k){
    ...
    for (int i = 0; i < n-k+1; i++) { // CHANGED
        ...
        for (int ik = 0; ik < k and i + ik <n; ik++) { // CHANGED
            std::vector<float>& profile_column = profile[i + ik];
            typename std::vector<float>::iterator max = std::max_element(profile_column.begin(), profile_column.end()); //std::vector<float>::iterator
            score += *max;
...
void enumerate_kmers_heuristic3(std::vector<std::vector<float>>& profile,std::size_t &n, std::size_t &s, std::string& alphabet, float T, int k,
    ...
    for (int a = 0; a < s; a++) {
        ...
        if (score > T) { // See comment in exhaustive search.
            ...
            if (ik == k - 1 or i + ik == n-1) { // or if i*k + ik < n, for the last k-mer.
                ...
                enumerate_kmers_heuristic3(profile, n,s, alphabet, T, k, strings, new_string,  score_cashe,  (ik + 1), i, best_string);
            }
        }
    }
}

auto heuristic3(std::vector<std::vector<float>> &  profile, std::size_t &n, std::size_t &s, std::string& alphabet, float T, int k=1){
    std::string best_string(n, ' ');
    std::vector<float> best_remaining_scores((int) n ); // CHANGED
    float best_score;
    get_best_string_heur3(profile, n,s, best_remaining_scores, best_string, best_score, alphabet, k);
    ...
    for (int i = 0; i < n-k+1; i++) { // CHANGED
        ...
        enumerate_kmers_heuristic3(profile, n,s, alphabet, T, k, strings,  new_string,  cashe, ik,  i, best_string);
    }
